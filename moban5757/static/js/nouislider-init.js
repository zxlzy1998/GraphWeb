//
// function range_to_value(slider,value){
...
(function($) {
    ...
    //read params list
        ...
        $.ajax({
            url: "graphdata/paramlist.json",
            success: function (data) {

                var stepSlider = new Array(data.length)
                var stepSliderValueElement = new Array(data.length)
                for (var i = 0; i < data.length; i++ ) {
                    var j = i + 1

                    stepSlider[i] = document.getElementById('slider-step-' + j);

                    noUiSlider.create(stepSlider[i], {
                        start: [data[i].min],
                        step: data[i].step,
                        range: {
                            'min': [data[i].min],
                    ...
                    stepSliderValueElement[i] = document.getElementById('slider-step-value-' + j);
                    // stepSlider[0].noUiSlider.on('update', function (values, handle) {
                    //     console.log( stepSlider[0])
                    //
                    //
                ...
                for (let i =0;i<3;i++){
                    stepSlider[i].noUiSlider.on('update', function (values, handle) {
                        stepSliderValueElement[i].innerHTML = values[handle];
                    });
                }
                // stepSlider[0].noUiSlider.on('update', function (values, handle) {
                //     stepSliderValueElement[0].innerHTML = values[handle];
                // });
                // stepSlider[1].noUiSlider.on('update', function (values, handle) {
                //     stepSliderValueElement[1].innerHTML = values[handle];
                // });
                // stepSlider[2].noUiSlider.on('update', function (values, handle) {
                //     stepSliderValueElement[2].innerHTML = values[handle];
                // });
